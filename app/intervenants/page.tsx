...
const domains = ['Tous', ...Array.from(new Set(mockSpeakers.map(s => s.domain)))]
...
export default function IntervenantsPage() {
  ...
  const filtered = mockSpeakers.filter(s => {
    ...
    const matchDomain = selectedDomain === 'Tous' || s.domain === selectedDomain
      ...
      <div className="bg-gradient-to-br from-primary-950 to-primary-900 py-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white mb-4">Nos Intervenants</h1>
            <div className="section-divider mx-auto mb-6" />
            <p className="text-white/70 max-w-2xl mx-auto">
              {mockSpeakers.length} experts de haut niveau issus de l'humanitaire, du développement et des institutions internationales
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-10">
          ...
          <div className="flex flex-wrap gap-2">
            {domains.slice(0, 5).map(d => (
              ...
              >
                {d === 'Tous' ? `Tous (${mockSpeakers.length})` : d}
              </button>
            ))}
          </div>
        </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-semibold">Aucun intervenant trouvé</p>
          </div>
        )}
      </div>
          ...
          >
            <div className="relative">
              ...
              <button
                ...
                className="absolute top-4 right-4 bg-white/20 backdrop-blur text-white w-9 h-9 rounded-full flex items-center justify-center hover:bg-white/40 transition-colors"
              >✕</button>
            ...
            <div className="p-6">
              <div className="flex gap-2 mb-4">
                <span className="bg-primary-50 text-primary-800 text-xs px-3 py-1 rounded-full font-bold">{selectedSpeaker.domain}</span>
                {selectedSpeaker.featured && <span className="bg-amber-50 text-amber-700 text-xs px-3 py-1 rounded-full font-bold">⭐ Intervenant vedette</span>}
              </div>
              <p className="text-gray-700 leading-relaxed mb-4">{selectedSpeaker.bio}</p>
              <div className="border-t border-gray-100 pt-4 flex items-center justify-between">
                <p className="text-sm text-gray-500">Sessions : {selectedSpeaker.sessions.length} panel(s)</p>
